    ...
    fetch('/api/user/auth')
      ...
      .then((data) => {
        if (data === 'not logged in') {
          setHeaderOptions(
            <>
              <a className={location.pathname === '/login' || location.pathname === '/login/' ? 'm-sm' : 'm-sm icon-opacity'} href="login">
                <strong>
                  <em>Login</em>
                </strong>
              </a>
          ...
          setHeaderOptions(
            <>
              {' '}
              <p className="m-sm icon-opacity">
                <strong>{data}</strong>
              </p>
              <a className="m-sm icon-opacity" onClick={postLogout}>
                <strong>
                  <em>Logout</em>
                </strong>
              </a>
        ...
        <nav>
          ...
          <div>
            <a className={location.pathname === '/' ? 'm-sm' : 'm-sm icon-opacity'} href="/">
              <svg className="logo-main" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512">
